...
const pickCategoryField = (schema) => {
...
export const aggregateByDateCount = (rows, dateFieldName) => {
...
export const aggregateByCategoryCount = (rows, categoryFieldName) => {
    const counts = new Map();
    for (const row of rows) {
...
export const chooseLargeDatasetFallback = (rows, schema) => {
    ...
    if (categoryFieldName)
        return aggregateByCategoryCount(rows, categoryFieldName);
