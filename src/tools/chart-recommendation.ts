...
export const toEncodingType = (fieldType: SchemaField["type"]): "nominal" | "quantitative" | "temporal" => {
  if (fieldType === "number") return "quantitative";
  if (fieldType === "date") return "temporal";
  return "nominal";
};

type RecommendationBuilder = (schema: InferredSchema) => ChartRecommendation;

const buildLineRecommendation: RecommendationBuilder = (schema) => {
  const dateField = schema.fields.find((f) => f.type === "date");
  const numberField = schema.fields.find((f) => f.type === "number");
  if (!dateField || !numberField) throw new Error("A line chart requires one date field and one numeric field.");
...
const buildBarRecommendation: RecommendationBuilder = (schema) => {
  const stringField = schema.fields.find((f) => f.type === "string");
  const numberField = schema.fields.find((f) => f.type === "number");
  if (!stringField || !numberField) throw new Error("A bar chart requires one categorical field and one numeric field.");
...
const buildScatterRecommendation: RecommendationBuilder = (schema) => {
  const numberFields = schema.fields.filter((f) => f.type === "number");
  if (numberFields.length < 2) throw new Error("A scatter chart requires two numeric fields.");
...
const buildTimelineRecommendation: RecommendationBuilder = (schema) => {
  const dateFields = schema.fields.filter((f) => f.type === "date");
  const stringFields = schema.fields.filter((f) => f.type === "string");
  if (dateFields.length < 2 || stringFields.length === 0) throw new Error("A timeline/gantt chart requires two date fields and one categorical field.");
  return {
    chartType: "timeline",
...
const buildAreaRecommendation: RecommendationBuilder = (schema) => {
  const dateField = schema.fields.find((f) => f.type === "date");
  const numberField = schema.fields.find((f) => f.type === "number");
  if (!dateField || !numberField) throw new Error("An area chart requires one date field and one numeric field.");
...
const buildHistogramRecommendation: RecommendationBuilder = (schema) => {
  const numberField = schema.fields.find((f) => f.type === "number");
  if (!numberField) throw new Error("A histogram requires at least one numeric field.");
...
const buildPieRecommendation: RecommendationBuilder = (schema) => {
  const stringField = schema.fields.find((f) => f.type === "string");
  const numberField = schema.fields.find((f) => f.type === "number");
  if (!stringField || !numberField) throw new Error("A pie chart requires one categorical field and one numeric field.");
...
const buildBoxplotRecommendation: RecommendationBuilder = (schema) => {
  const stringField = schema.fields.find((f) => f.type === "string");
  const numberField = schema.fields.find((f) => f.type === "number");
  if (!stringField || !numberField) throw new Error("A boxplot requires one categorical field and one numeric field.");
...
const buildHeatmapRecommendation: RecommendationBuilder = (schema) => {
  const stringFields = schema.fields.filter((f) => f.type === "string");
  const numberField = schema.fields.find((f) => f.type === "number");
  if (stringFields.length < 2 || !numberField) throw new Error("A heatmap requires two categorical fields and one numeric field.");
...
const RECOMMENDATION_BUILDERS: Partial<Record<SupportedChartType, RecommendationBuilder>> = {
...
const autoDetectRecommendation: RecommendationBuilder = (schema) => {
  const dateField = schema.fields.find((f) => f.type === "date");
  const numberFields = schema.fields.filter((f) => f.type === "number");
  const stringFields = schema.fields.filter((f) => f.type === "string");
  const dateFields = schema.fields.filter((f) => f.type === "date");

  if (dateField && numberFields.length > 0) return buildLineRecommendation(schema);
  if (stringFields.length > 0 && numberFields.length > 0) return buildBarRecommendation(schema);
  if (numberFields.length >= 2) return buildScatterRecommendation(schema);
  if (dateFields.length >= 2 && stringFields.length > 0) return buildTimelineRecommendation(schema);

  throw unsupportedDatasetError(schema);
};

export const chooseRecommendation = (schema: InferredSchema, requestedChartType?: SupportedChartType): ChartRecommendation => {
  if (!requestedChartType) return autoDetectRecommendation(schema);
  const builder = RECOMMENDATION_BUILDERS[requestedChartType];
  if (!builder) return autoDetectRecommendation(schema);
