import type { InferredSchema, SchemaField } from "../types.js";

const joinFieldNames = (fields: SchemaField[]): string =>
  fields.length > 0 ? fields.map((f) => f.name).join(", ") : "none";

export const getUnsupportedDatasetSuggestions = (schema: InferredSchema, largeDatasetMode = false): string[] => {
...
export const unsupportedDatasetError = (schema: InferredSchema): Error => {
  const dateFields = schema.fields.filter((f) => f.type === "date");
...
export const largeDatasetRequiresTransformationError = (
